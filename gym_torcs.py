...
class TorcsEnv:
    ...
    def __init__(self, vision=False, throttle=False, gear_change=False, return_np=False):
       #print("Init")
        self.vision = vision
    ...
    def step(self, u):
       #print("Step")
        ...
        if client.R.d['meta'] is True: # Send a reset signal
            ...
            info.update({'time':time, 'distFromStart':dist, 'totalDistFromStart':totDist,
                    'distRaced':distraced, 'lastLapTime':lastLapTime})
            client.respond_to_server()
    ...
    def make_observation(self, raw_obs, return_np=False):
        ...
        if return_np:
            ob = np.concatenate((np.array(raw_obs['focus'], dtype=np.float32).reshape(-1)/200,
                                ...
                                np.array(raw_obs['speedY'], dtype=np.float32).reshape(-1)/self.default_speed,
                                np.array(raw_obs['speedZ'], dtype=np.float32).reshape(-1)/self.default_speed,
                                np.array(raw_obs['rpm'], dtype=np.float32).reshape(-1),
                                np.array(raw_obs['track'], dtype=np.float32).reshape(-1)/200.,
